...
class tx_amazonaffiliate_browselinkshook implements \TYPO3\CMS\Core\ElementBrowser\ElementBrowserHookInterface {
	 ...
	 *
	 * @param browse_links $parentObject browse_links object
	 * @param array $additionalParameters parameters
	 * @return void
	 */
	 ...
	 * @return	array	$currentlyAllowedItems currently allowed items plus added items
	 */
	...
	public function getTab($linkSelectorAction) {
		...
		if($linkSelectorAction == 'amazon_affiliate') {
			...
			if(stripos($this->pObj->curUrlInfo['value'], 'amazonaffiliate:') !== false) {
				$value = substr($this->pObj->curUrlInfo['value'], stripos($this->pObj->curUrlInfo['value'], 'amazonaffiliate:') + strlen('amazonaffiliate:'));
			} elseif(stripos($this->pObj->curUrlInfo['value'], 'http://') !== false) {
				$value = substr($this->pObj->curUrlInfo['value'], stripos($this->pObj->curUrlInfo['value'], 'http://') + strlen('http://'));
			} else {
			...
			$content = '
						<script type="text/javascript">
							...
							' . ($this->isRTE()?'browse_links_setHref(urlValue); browse_links_setAdditionalValue(\'external\', \'\'); return link_current();':'return link_folder(urlValue);').'
								...
								<form action="" name="lurlform" id="lurlform">
									<table border="0" cellpadding="2" cellspacing="1" id="typo3-linkURL">
										<tr>
											...
											<td><input type="text" name="asin"' . $this->pObj->doc->formWidth(30) . ' value="' . htmlspecialchars($url) . '" /> ' .
					'<input type="submit" value="' . $GLOBALS['LANG']->getLL('setLink', 1) . '" onclick="return link_asin('.$prefix.'document.lurlform.asin.value, document.lurlform.amazonLinkHover.checked);" /></td>
										</tr>
										<tr>
											<td style="width: 96px;">Hover:</td>
											<td><input type="checkbox" name="amazonLinkHover" value="1" ' . ($amazonLinkHover == 1 ? 'checked="checked"' : "") . ' /></td>
										</tr>
									</table>
								</form>';
	...
	public function parseCurrentUrl($href, $siteUrl, $info) {
		...
		if(strtolower(substr($href, 0, 15)) == 'amazonaffiliate') {
			\TYPO3\CMS\Core\Utility\GeneralUtility::_GETset('amazon_affiliate','act');
