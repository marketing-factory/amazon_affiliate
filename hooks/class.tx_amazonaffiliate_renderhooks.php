...
class tx_amazonaffiliate_renderhooks extends tx_cssstyledcontent_pi1 {
	...
	public function render_textpic($content, $conf) {
		...
		if ($amazonImgList == '') {
			...
			$amazonProductsTempArray = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(LF, $amazonImgList);
			foreach ($amazonProductsTempArray as $amazonProduct) {
				if (\TYPO3\CMS\Core\Utility\GeneralUtility::isFirstPartOfStr($amazonProduct, "amazonaffiliate:")) {
					$amazonProduct = substr($amazonProduct, strlen("amazonaffiliate:"));
				}
				$amazonProduct = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode("|", $amazonProduct);
			...
			if ($equalHeight) {
				...
				for ($a = 0; $a < $imgCount; $a++) {
					...
					$amazonProduct = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('tx_amazonaffiliate_product', $amazonProducts[$imgKey]['asin']);
					$imgInfo = array(
			...
			for ($a = 0; $a < $imgCount; $a++) {
				...
				if ($cols == 1) {
					if ($this->cObj->data['imagewidth'] > 0) {
						...
						$amazonProduct = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('tx_amazonaffiliate_product', $amazonProducts[$a]['asin']);
				...
				$imgsTag[$imgKey] = $amazonEcs->getAmazonImageOnlyCode($amazonProducts[$a]['asin'], $imageRowsFinalWidths[floor($a % $colCount)], $imgConf['file.']['height'], $amazonProducts[$a]['hover']);
