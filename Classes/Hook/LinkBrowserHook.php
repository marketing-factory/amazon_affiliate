 ...
 */
class LinkBrowserHook implements \TYPO3\CMS\Core\ElementBrowser\ElementBrowserHookInterface
{
     ...
     *
     * @param ElementBrowser $parentObject browse_links object
     * @return void
     */
    ...
    public function getTab($linkSelectorAction)
    {
        ...
        if ($linkSelectorAction == 'amazon_affiliate') {
            ...
            if (stripos($this->pObj->curUrlInfo['value'], 'amazonaffiliate:') !== false) {
                $value = substr($this->pObj->curUrlInfo['value'],
                ...
                $value = substr($this->pObj->curUrlInfo['value'],
                    stripos($this->pObj->curUrlInfo['value'], 'http://') + strlen('http://'));
            } else {
...
'<input type="submit" value="' . $this->getLanguageService()->getLL('setLink',
    1) . '" onclick="return link_asin(' . $prefix . 'document.lurlform.asin.value, document.lurlform.amazonLinkHover.checked);" /></td>
                        </tr>
                        <tr>
    ...
    public function parseCurrentUrl($href, $siteUrl, $info)
    {
        ...
        if (strtolower(substr($href, 0, 15)) == 'amazonaffiliate') {
            GeneralUtility::_GETset('amazon_affiliate', 'act');
